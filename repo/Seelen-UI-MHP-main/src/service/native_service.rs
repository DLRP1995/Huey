// the code in this file is for reference only, it is not used in the actual service
// like there's issue on different installers like msix, so we spawn the executable directly

use crate::string_utils::WindowsString;
...
use crate::{error::Result, SERVICE_DISPLAY_NAME, SERVICE_NAME};

lazy_static! {
    static ref SLU_SERVICE: Arc<Mutex<SluNativeService>> = Arc::new(Mutex::new(SluNativeService::new()));
}

/// All documentation is taken from https://learn.microsoft.com/en-us/windows/win32/services/services
#[allow(dead_code)]
struct SluNativeService {
    handle: SERVICE_STATUS_HANDLE,
    status: SERVICE_STATUS,
    stop_event: HANDLE,
}

unsafe impl Send for SluNativeService {}

#[allow(dead_code)]
impl SluNativeService {
